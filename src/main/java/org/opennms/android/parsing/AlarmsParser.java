            ...
            if (nodes != null) {
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if (firstEventTime != null) {
                        values.put(Contract.Alarms.FIRST_EVENT_TIME,
                                   firstEventTime.getTextContent());
                    }
                    ...
                    Node lastEventSeverity =
                            getXmlNodeForExpression("lastEvent/@severity", currentNode);
                    if (lastEventSeverity != null) {
                        values.put(Contract.Alarms.LAST_EVENT_SEVERITY,
                                   lastEventSeverity.getTextContent());
                    }

                    Node nodeId = getXmlNodeForExpression("nodeId", currentNode);
                    if (nodeId != null) {
                        values.put(Contract.Alarms.NODE_ID,
                    ...
                    if (serviceTypeName != null) {
                        values.put(Contract.Alarms.SERVICE_TYPE_NAME,
