        ...
        try {
            ...
            if (nodes != null) {
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    values.put(Contract.Events.COLUMN_EVENT_ID, id);

                    String description = getXmlNodeForExpression("description", currentNode).getTextContent();
                    if (description != null) values.put(Contract.Events.COLUMN_DESCRIPTION, description);

                    String logMessage = getXmlNodeForExpression("logMessage", currentNode).getTextContent();
                    if (logMessage != null) values.put(Contract.Events.COLUMN_LOG_MESSAGE, logMessage);

                    String severity = getXmlNodeForExpression("@severity", currentNode).getNodeValue();
                    if (severity != null) values.put(Contract.Events.COLUMN_SEVERITY, severity);

                    String host = getXmlNodeForExpression("host", currentNode).getTextContent();
                    if (host != null) values.put(Contract.Events.COLUMN_HOST, host);

                    String ipAddress = getXmlNodeForExpression("ipAddress", currentNode).getTextContent();
                    if (ipAddress != null) values.put(Contract.Events.COLUMN_IP_ADDRESS, ipAddress);

                    String nodeId = getXmlNodeForExpression("nodeId", currentNode).getTextContent();
                    if (nodeId != null) values.put(Contract.Events.COLUMN_NODE_ID, Integer.parseInt(nodeId));

                    String nodeLabel = getXmlNodeForExpression("nodeLabel", currentNode).getTextContent();
                    if (nodeLabel != null) values.put(Contract.Events.COLUMN_NODE_LABEL, nodeLabel);

                    String createTime = getXmlNodeForExpression("createTime", currentNode).getTextContent();
                    if (createTime != null) values.put(Contract.Events.COLUMN_CREATE_TIME, createTime);

                    String serviceTypeId = getXmlNodeForExpression("serviceType/@id", currentNode).getTextContent();
                    if (serviceTypeId != null)
                        values.put(Contract.Events.COLUMN_SERVICE_TYPE_ID, Integer.parseInt(serviceTypeId));

                    String serviceTypeName = getXmlNodeForExpression("serviceType/name", currentNode).getTextContent();
                    if (serviceTypeName != null) values.put(Contract.Events.COLUMN_SERVICE_TYPE_NAME, serviceTypeName);
